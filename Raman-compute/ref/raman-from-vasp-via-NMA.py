...
def parse_poscar(poscar_fh):
    ...
    b = []
    for i in range(2, 5):
        b.append([float(x)*scale for x in lines[i].split()[:3]])  #三个基矢量*scale
    #
    vol = b[0][0]*b[1][1]*b[2][2] + b[1][0]*b[2][1]*b[0][2] + b[2][0]*b[0][1]*b[1][2] - \
          b[0][2]*b[1][1]*b[2][0] - b[2][1]*b[1][2]*b[0][0] - b[2][2]*b[0][1]*b[1][0]    #det(b)计算三个向量定义的体积
    #
    try:
        num_atoms = [int(x) for x in lines[5].split()]  #读取原子数
        line_at = 6
    except ValueError:
...
def parse_env_params(params):
    ...
    if len(tmp) != 4:
        ...
        sys.exit(1)
    #
    [first, last, nderiv, step_size] = [int(tmp[0]), int(tmp[1]), int(tmp[2]), float(tmp[3])]  #first-last,NMs个数，ndervi导数阶数,step_size坐标微扰幅度
    #
...
def parse_modesdat(modesdat_fh, nat):
    ...
    for i in range(nat*3): # all frequencies should be supplied, regardless of requested to calculate
        ...
        eigvecs[i] = eigvec   #A*amu^1/2
        norms[i] = sqrt( sum( [abs(x)**2 for sublist in eigvec for x in sublist] ) )  #1/amu^1/2

    return eigvecs, norms  #读取特征向量/NCs和范数/向量长度
#### end subs for VTST
#
def get_modes_from_OUTCAR(outcar_fh, nat):  #vasp计算介电常数，直接给出质权hessian的特征值和特征向量
    from math import sqrt
    eigvals = [ 0.0 for i in range(nat*3) ]
    eigvecs = [ 0.0 for i in range(nat*3) ]
    norms   = [ 0.0 for i in range(nat*3) ]
        ...
        if "Eigenvectors after division by SQRT(mass)" in line:
            ...
            for i in range(nat*3): # all frequencies should be supplied, regardless of those requested to calculate
                ...
                for j in range(nat):
                    ...
                    #
                eigvecs[i] = eigvec
                norms[i] = sqrt( sum( [abs(x)**2 for sublist in eigvec for x in sublist] ) )  #对每个原子的特征向量进行遍历加和，最后开平方
            #
            return eigvals, eigvecs, norms  #读取质量加权下的NCs
        #
...
if __name__ == '__main__':
    ...
    for i in range(first-1, last):
        ...
        for j in range(len(disps)):
            disp_filename = 'OUTCAR.%04d.%+d.out' % (i+1, disps[j]) #disps = [-1, 1] 
            #
            try:
                ...
                if args['use_poscar'] != True:
                    ...
                    poscar_fh.write("%s %4.1e \n" % (disp_filename, step_size)) #step_size=0.01
                    poscar_fh.write(poscar_header)
                    poscar_fh.write("Cartesian\n")
                    #
                    for k in range(nat):  #nat=natoms
                        pos_disp = [ pos[k][l] + eigvec[k][l]*step_size*disps[j]/norm for l in range(3)]  #原坐标+特征向量*0.01*±1/norm
